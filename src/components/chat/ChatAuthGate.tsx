...
export default function ChatAuthGate({
  ...
  useEffect(() => {
    ...
    if (currentAppUserId !== null && currentAppUserId !== appUser.id) {
      console.log(
        "ChatAuthGate: App user changed from",
        currentAppUserId,
        "to",
        appUser.id,
        "- forcing Firebase signout"
      );
      setReady(false);
      ...
      signOut(auth)
        .then(() => {
          console.log(
            "ChatAuthGate: Firebase signout completed, will create new user"
          );
        })
        .catch((error) => {
          console.error(
            "ChatAuthGate: Error signing out from Firebase:",
            error
          );
        });
      return;
    ...
    const unsub = onAuthStateChanged(auth, async (fbUser) => {
      if (fbUser && fbUser.uid !== expectedUid) {
        console.log(
          "ChatAuthGate: UID mismatch, expected",
          expectedUid,
          "got",
          fbUser.uid,
          "- signing out"
      ...
      if (!fbUser) {
        console.log(
          "ChatAuthGate: Creating Firebase token for user:",
          appUser.id
        );
        ...
        if (!res.ok) {
          console.error(
            "ChatAuthGate: Failed to get Firebase token:",
            res.status
          );
      ...
      console.log(
        "ChatAuthGate: Firebase user signed in with UID:",
        fbUser.uid
      );
      console.log(
        "ChatAuthGate: Updating Firestore user document for app user:",
        appUser?.id
      );
      ...
      console.log(
        "ChatAuthGate: User document updated, chat ready for app user:",
        appUser?.id,
        "Firebase UID:",
        fbUser.uid
      );
